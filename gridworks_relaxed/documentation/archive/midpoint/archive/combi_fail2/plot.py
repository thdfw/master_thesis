...
'''
def print_iteration(u_opt, x_opt, x_1, pb_type, combi):
    ...
    m_HP = round(functions.get_function("m_HP", u_opt_0, x_opt_0, 0, True, False, 0, combi),2) if u_opt_0[4]==1 else 0
    m_stor = round(u_opt[1,0],1)
    m_buffer = round(functions.get_function("m_buffer", u_opt_0, x_opt_0, 0, True, False, 0, combi),1)
    m_load = 0.2
    ...
    for _ in range(pb_type['eta']+1):
        ...
        Q_HP = optimizer.get_function("Q_HP", u_opt_0, x_inter, 0, True, False, 0, combi)
        T_ret_HP = functions.get_function("T_ret_HP", u_opt_0, x_inter, 0, True, False, 0, combi)
        T_sup_load = functions.get_function("T_sup_load", u_opt_0, x_inter, 0, True, False, 0, combi)
        
        # Go to next intermediate point
        x_inter = optimizer.dynamics(u_opt_0, x_inter, 0, True, False, pb_type['eta'], pb_type['delta_t_m']*60, 0, combi)
    ...
    Q_HP_average = optimizer.get_Q_HP_t(u_opt_0, x_opt_0, 0, True, False, pb_type['eta'], pb_type['delta_t_m']*60, 0, combi)
