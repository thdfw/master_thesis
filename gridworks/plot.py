...
def print_iteration(u_opt, x_opt, x_1, pb_type, sequence, iter):
    ...
    m_buffer = round(m_buffer/45, 1)
    m_stor = round(m_stor/45, 1)
    ...
    # print(f"Resistive elements: {[round(float(x),1) for x in u_opt[5,0:45]]}\n")
...
def plot_MPC(data):
    ...
    ax[0].set_xlim([0,45*data['iterations']])
    ...
    # x_ticks in hours
    tick_positions = np.arange(0, data['iterations']*45+1, step=int(60/data['pb_type']['time_step']))
    ...
    ax[1].plot((data['iterations']*45+1)*[273+38], color='black', label="$T_{sup,load,min}$", alpha=0.2, linestyle='dotted')
...
def plot_single_iter(data):
    ...
    ax[1].plot(data['T_B1'], color='blue', label="$T_{B1}$", alpha=0.4)
    ax[1].plot(data['T_B4'], color='blue', label="$T_{B4}$", alpha=0.4, linestyle='dashed')
    ax[1].plot((180)*[273+38], color='black', label="$T_{sup,load,min}$", alpha=0.2, linestyle='dotted')
    ax[1].set_ylabel("Temperatuere [K]")
