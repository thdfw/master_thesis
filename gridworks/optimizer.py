...
def optimize_N_steps(x_0, a, iter, pb_type, sequence, warm_start, PRINT):
    ...
    if PRINT:
        print("\n-----------------------------------------------------")
        print(f"Iteration {iter/30+1} ({hours}h{minutes}min)")
        print("-----------------------------------------------------\n")
    ...
    for t in range(N):
        ...
        if t>=0  and t<30: [d_ch, d_bu, d_HP] = sequence['combi1']
        if t>=30 and t<60: [d_ch, d_bu, d_HP] = sequence['combi2']
        if t>=60 and t<90: [d_ch, d_bu, d_HP] = sequence['combi3']
        if t>=90 and t<120: [d_ch, d_bu, d_HP] = sequence['combi4']
        ...
        if PRINT:
            ...
            if t==30: print(f"0h30-1h00: {sequence['combi2']}")
            if t==60: print(f"1h00-1h30: {sequence['combi3']}")
            if t==90: print(f"1h30-2h00: {sequence['combi4']}\n")
