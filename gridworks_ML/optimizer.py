...
def optimize_N_steps(x_0, a, iter, pb_type, combi):
    ...
    print(f"Combi for next N steps: {combi}")
    ...
    for t in range(N):
        ...
        # delta terms for the requested combination
        d_ch_fixed = combi[f'timestep{t}'][0]
        d_bu_fixed = combi[f'timestep{t}'][1]
        d_HP_fixed = d_ch_fixed
    ...
    for t in range(N):
        ...
        # Heat pump operation (if the heat pump is not fixed to be off)
        if combi[f'timestep{t}'][0] == 1:
            opti.subject_to(get_function("Q_HP", u[:,t], x[:,t], a, real, approx, t, combi) >= Q_HP_min * u[4,t])
            opti.subject_to(get_function("Q_HP", u[:,t], x[:,t], a, real, approx, t, combi) <= Q_HP_max)
        
        # Load supply temperature
        opti.subject_to(get_function("T_sup_load", u[:,t], x[:,t], a, real, approx, t, combi) >= T_sup_load_min)
    ...
    obj = sum(c_el[t] * delta_t_h * get_function("Q_HP", u[:,t], x[:,t], a, real, approx, t, combi) for t in range(N))
